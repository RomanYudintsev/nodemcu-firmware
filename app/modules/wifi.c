...
static void wifi_event_handle_wpa2_event_cb(System_Event_t *evt)
{
  switch (evt->event) {
...
static int wifi_station_config( lua_State* L )
{
  ...
  if(lua_istable(L, 1))
  {
    ...
    if (!lua_isnil(L, -1))
    {
      if (lua_isboolean(L, -1))
      {
        ...
        if (is_wpa2)
        {
          ...
          memset(wpa2_pwd, 0, sizeof(wpa2_pwd));
        }
      }
      else
      {
    ...
    if (!lua_isnil(L, -1))
    {
      if( lua_isstring(L, -1) )
      {
        ...
        if (is_wpa2)
        {
          memcpy(wpa2_pwd, pwd, pl);
          memcpy(sta_conf.password, "", sizeof(sta_conf.password));
        }
        else
        {
    ...
    if (is_wpa2)
    {
      lua_getfield(L, 1, "login");
        if (!lua_isnil(L, -1))
        {
          if( lua_isstring(L, -1) )
          {
            const char *lgn = luaL_checklstring( L, -1, &wll );
            luaL_argcheck(L, ((wll>=0 && wll<=sizeof(wpa2_login)) ), 1, "login: length:0-64");
            memcpy(wpa2_login, lgn, wll);
          }
          else
          {
  ...
  if (is_wpa2)
  {
    ...
    memset(debug_temp, 0, sizeof(debug_temp));
    memcpy(debug_temp, wpa2_pwd, sizeof(wpa2_pwd));
    WIFI_DBG("\twpa2_pwd=\"%s\" len=%d\n", debug_temp, pl);
    memset(debug_temp, 0, sizeof(0));
  }
  ...
  #if defined(WIFI_DEBUG)
  WIFI_DBG("\twifi_station_set_config=%s\n", config_success ? "true":"false");
  #endif
  if(is_wpa2)
  {
    os_printf("clear wpa2 credentials");
    ...
    unsigned int wpa2_result = wifi_station_set_wpa2_enterprise_auth(1);
    #if defined(WIFI_DEBUG)
    WIFI_DBG("\twifi_station_set_wpa2_enterprise_auth=%d\n", wpa2_result);
    #endif
    wpa2_result = wifi_station_set_enterprise_identity((uint8*)wpa2_login, strlen(wpa2_login));
    #if defined(WIFI_DEBUG)
    WIFI_DBG("\twifi_station_set_enterprise_identity=%d\n", wpa2_result);
    #endif
    wpa2_result = wifi_station_set_enterprise_username((uint8*)wpa2_login, strlen(wpa2_login));
    #if defined(WIFI_DEBUG)
    WIFI_DBG("\twifi_station_set_enterprise_username=%d\n", wpa2_result);
    #endif
    wpa2_result = wifi_station_set_enterprise_password((uint8*)wpa2_pwd, strlen(wpa2_pwd));
    #if defined(WIFI_DEBUG)
    WIFI_DBG("\twifi_station_set_enterprise_password=%d\n", wpa2_result);
    #endif
    auto_connect = false;
    wifi_station_set_auto_connect((uint8)0);
  }
